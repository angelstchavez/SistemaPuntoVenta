using Entidad.Registros;
using System.Collections.Generic;
using System.IO;
using System;
using Seguridad;

namespace Datos.Archivos
{
    public class ArchivoVenta
    {
        protected string ruta = "ventas.dat";

        public ArchivoVenta()
        {
            this.ruta = "ventas.dat";
        }

        public ArchivoVenta(string ruta)
        {
            this.ruta = ruta;
        }

        public bool GuardarDatos(Venta venta)
        {
            try
            {
                StreamWriter sw = new StreamWriter(ruta, true);
                sw.WriteLine(Encriptacion.Encrypt(venta.ToString()));
                sw.Close();
                return true;
            }
            catch (Exception ex) { throw ex; }
        }

        public bool update(List<Venta> venta)
        {
            try
            {
                StreamWriter sw = new StreamWriter("temporal", true);
                foreach (var item in venta) { sw.WriteLine(item); }
                sw.Close();
                File.Delete(ruta);
                File.Move("temporal", ruta);
                return true;
            }
            catch (Exception ex) { throw ex; }
        }

        public void vaciar()
        {
            StreamWriter sw = new StreamWriter(ruta, false);
            sw.Write("");
            sw.Close();
        }
    }
}
